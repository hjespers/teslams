...
(function($){
    function init(plot){
		...
		var j = 0, k = series[i].data.length - 1;
		while (j <= k && series[i].data[j] === null)
		    j++;
		while (j <= k && series[i].data[k] === null)
		    k--;
                minMaxValues[0] = series[i].data[j][AxeIdx];
                minMaxValues[1] = series[i].data[k][AxeIdx];
            }
            return minMaxValues;
        }
